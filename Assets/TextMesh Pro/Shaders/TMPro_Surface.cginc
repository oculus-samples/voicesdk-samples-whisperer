void VertShader(inout appdata_full v, out Input data) {
  ...
  pixelSize /= float2(_ScaleX, _ScaleY) * mul(
  ...
  scale = lerp(
      ...
      abs(
          dot(
              UnityObjectToWorldNormal(v.normal.xyz),
...
void PixShader(Input input, inout SurfaceOutput o) {
  ...
  faceColor *= tex2D(
      _FaceTex,
      float2(
          input.uv2_FaceTex.x + _FaceUVSpeedX * _Time.y,
          input.uv2_FaceTex.y + _FaceUVSpeedY * _Time.y));
  outlineColor *= tex2D(
      _OutlineTex,
      float2(
          input.uv2_OutlineTex.x + _OutlineUVSpeedX * _Time.y,
