...
namespace Meta.Conduit
{
    ...
    {
        protected Dictionary<string, object> ActualParameters = new();

        /// <summary>
        ///     Maps internal parameter names to fully qualified parameter names (roles/slots).
        /// </summary>
        private Dictionary<string, string> parameterToRoleMap = new();
        ...
        public bool ContainsParameter(ParameterInfo parameter)
        {
            return (ActualParameters.ContainsKey(parameter.Name) &&
                    parameterToRoleMap.ContainsKey(parameter.Name) &&
                    ActualParameters[parameter.Name].GetType() == parameter.ParameterType) ||
                   SupportedSpecializedParameter(parameter);
        }
        ...
        public object GetParameterValue(ParameterInfo formalParameter)
        {
            ...
            {
                if (!parameterToRoleMap.ContainsKey(formalParameterName))
                {
                    Debug.LogError($"Parameter {formalParameterName} is missing");
                    return false;
                }

                formalParameterName = parameterToRoleMap[formalParameterName];
            }

            //var parameterValue = this.ActualParameters[formalParameterName];
            if (ActualParameters.TryGetValue(formalParameterName, out var parameterValue))
            {
                if (formalParameter.ParameterType == typeof(string))
                    return parameterValue;
                if (formalParameter.ParameterType.IsEnum)
                    try
                    {
            ...
            if (SupportedSpecializedParameter(formalParameter)) return GetSpecializedParameter(formalParameter);
