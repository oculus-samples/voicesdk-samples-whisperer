...
namespace Meta.Conduit
{
    ...
    internal class ConduitDispatcher : IConduitDispatcher
    {
        ...
        /// </summary>
        private readonly Dictionary<string, string> parameterToRoleMap = new();
        ...
        public bool InvokeAction(string actionId, Dictionary<string, object> parameters, float confidence = 1f,
            bool partial = false)
        {
            if (!manifest.ContainsAction(actionId)) return false;

            parameterProvider.Populate(parameters, parameterToRoleMap);

            var invocationContexts = ResolveInvocationContexts(actionId, confidence, partial);
            if (invocationContexts.Count < 1) return false;

            var allSucceeded = true;
            foreach (var invocationContext in invocationContexts)
                try
                {
                    if (!InvokeMethod(invocationContext)) allSucceeded = false;
                }
                catch (Exception e)
                {
        ...
        private List<InvocationContext> ResolveInvocationContexts(string actionId, float confidence, bool partial)
        {
            ...
            return invocationContexts.Where(context => CompatibleInvocationContext(context, confidence, partial))
                .ToList();
        }
        ...
        private bool CompatibleInvocationContext(InvocationContext invocationContext, float confidence, bool partial)
        {
            ...
            if (invocationContext.ValidatePartial != partial) return false;
            ...
            if (!parameters.All(parameter => parameterProvider.ContainsParameter(parameter)))
            {
                Debug.LogError(
                    $"Failed to find execution context for {invocationContext.MethodInfo.Name}. Parameters could not be matched");
                return false;
            }
        ...
        private bool InvokeMethod(InvocationContext invocationContext)
        {
            ...
            for (var i = 0; i < formalParametersInfo.Length; i++)
            {
                ...
                {
                    Debug.LogError(
                        $"Failed to find parameter {formalParametersInfo[i].Name} while invoking {method.Name}");
                    return false;
                }
            ...
            var allSucceeded = true;
            foreach (var obj in instanceResolver.GetObjectsOfType(invocationContext.Type))
                try
                {
