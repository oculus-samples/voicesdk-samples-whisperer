...
namespace Facebook.WitAi
{
    public static class WitEditorUI
    {
        ...
        public static void LayoutWindow(string windowTitle, Texture2D windowHeader, string windowHeaderUrl,
            Action windowContentLayout, ref Vector2 offset, out Vector2 size)
        {
            // Get minimum width
            var minWidth = WitStyles.WindowMinWidth;
            ...
            // Layout header image
            if (windowHeader != null) LayoutHeaderButton(windowHeader, windowHeaderUrl);
            // Layout header label
            if (!string.IsNullOrEmpty(windowTitle)) LayoutHeaderLabel(windowTitle);
            // End Header
            ...
            // Return size
            size = new Vector2(
                minWidth + WitStyles.WindowPaddingLeft + WitStyles.WindowPaddingRight + WitStyles.WindowScrollBarSize,
                WitStyles.WindowMinHeight);
        }
        ...
        public static void LayoutKeyObjectLabels(string key, object obj)
        {
            ...
            // Foldout
            var foldoutVoice = LayoutFoldout(new GUIContent(key), obj);
            if (!foldoutVoice) return;
            // Iterate fields
            EditorGUI.indentLevel++;
            foreach (var field in obj.GetType().GetFields()) LayoutKeyLabel(field.Name, field.GetValue(obj).ToString());
            EditorGUI.indentLevel--;
        }
        ...
        public static bool LayoutTextButton(string text)
        {
            ...
            var width = WitStyles.TextButton.CalcSize(content).x + WitStyles.TextButtonPadding * 2f;
            return LayoutButton(content, WitStyles.TextButton, new[] { GUILayout.Width(width) });
        }
        ...
        public static void LayoutTabButtons(string[] tabTitles, ref int selection)
        {
            if (tabTitles != null)
            {
                ...
                for (var i = 0; i < tabTitles.Length; i++)
                {
                    ...
                    if (LayoutTabButton(tabTitles[i])) selection = i;
                }
        ...
        public static void LayoutHeaderButton(Texture2D headerTexture, string headerURL)
        {
            if (headerTexture != null)
            {
                ...
                var maxWidth = EditorGUIUtility.currentViewWidth - WitStyles.WindowPaddingLeft -
                               WitStyles.WindowPaddingRight - WitStyles.IconButton.CalcSize(WitStyles.HelpIcon).x;
                var headerWidth = Mathf.Min(WitStyles.HeaderWidth, maxWidth);
                var headerHeight = headerWidth * headerTexture.height / headerTexture.width;
                if (GUILayout.Button(headerTexture, WitStyles.HeaderButton, GUILayout.Width(headerWidth),
                        GUILayout.Height(headerHeight)) &&
                    !string.IsNullOrEmpty(headerURL)) Application.OpenURL(headerURL);
                GUILayout.FlexibleSpace();
                if (LayoutIconButton(WitStyles.HelpIcon)) Application.OpenURL(headerURL);
                GUILayout.EndHorizontal();
            }
        }
        ...
        {
            var foldoutID = GetFoldoutID(bindObject);
            if (!string.IsNullOrEmpty(foldoutID) && foldouts.ContainsKey(foldoutID)) return foldouts[foldoutID];
            return false;
        }
        ...
        {
            var foldoutID = GetFoldoutID(bindObject);
            if (!string.IsNullOrEmpty(foldoutID)) foldouts[foldoutID] = toFoldout;
        }

        public static bool LayoutFoldout(GUIContent key, object bindObject)
        {
            var wasFoldout = GetFoldoutValue(bindObject);
            var isFoldout = LayoutFoldout(key, wasFoldout);
            if (isFoldout != wasFoldout) SetFoldoutValue(bindObject, isFoldout);
            return isFoldout;
        }
        ...
        {
            // Simple layout
            var newFieldValue = EditorGUILayout.IntField(key, fieldValue, WitStyles.IntField);
        ...
        {
            // Determine if locked
            var fieldId = GetFoldoutID(key.text);
            var isEditing = unlockFieldId.Equals(fieldId);
            ...
            // Layout
            var newFieldValue = isEditing ? unlockFieldText : fieldValue;
            var newUpdate = false;
            ...
            // Updated
            if (newUpdate && isEditing) unlockFieldText = newFieldValue;
        ...
        {
            // Simple layout
            var newToggleValue = EditorGUILayout.Toggle(key, toggleValue, WitStyles.Toggle);
        ...
        {
            // Default
            var newSelectionValue = selectionValue;
        ...
        public static bool LayoutSerializedObjectPopup(SerializedObject serializedObject, string propertyName,
