...
namespace Facebook.WitAi.Data.Configuration
{
    public static class WitConfigurationUtility
    {
        ...
        public static WitConfiguration[] WitConfigs
        {
            ...
            {
                // Reload if not setup
                if (_witConfigs == null) ReloadConfigurationData();
                // Force reload
                if (_needsConfigReload) ReloadConfigurationData();
                // Return config data
                return _witConfigs;
            }
        }
        ...
        private static WitConfiguration[] _witConfigs;

        // Wit configuration asset names
        public static string[] WitConfigNames { get; private set; } = Array.Empty<string>();

        // Config reload
        private static bool _needsConfigReload;
        ...
        public static void ReloadConfigurationData()
        {
            ...
            var found = new List<WitConfiguration>();
            var guids = AssetDatabase.FindAssets("t:WitConfiguration");
            foreach (var guid in guids)
            {
                var path = AssetDatabase.GUIDToAssetPath(guid);
                var config = AssetDatabase.LoadAssetAtPath<WitConfiguration>(path);
                if (!config.isDemoOnly) found.Add(config);
            }

            // Store wit configuration data
            _witConfigs = found.ToArray();
            // Obtain all names
            WitConfigNames = new string[_witConfigs.Length];
            for (var i = 0; i < _witConfigs.Length; i++) WitConfigNames[i] = _witConfigs[i].name;
        }
        ...
        {
            if (configuration != null && configuration.application != null) return configuration.application.id;
            return string.Empty;
        }
        ...
        public static int CreateConfiguration(string serverToken)
        {
            ...
            var configurationAsset = ScriptableObject.CreateInstance<WitConfiguration>();
            configurationAsset.name = WitTexts.Texts.ConfigurationFileNameLabel;
            configurationAsset.clientAccessToken = string.Empty;
            // Create
            var index = SaveConfiguration(serverToken, configurationAsset);
            if (index == -1) Object.DestroyImmediate(configurationAsset);
            // Return new index
            return index;
        }
            ...
            if (Selection.activeObject)
            {
                ...
                if (selectedPath.StartsWith("Assets"))
                {
                    if (AssetDatabase.IsValidFolder(selectedPath))
                        ...
                        rootDirectory = new FileInfo(selectedPath).DirectoryName;
                }
            }
        ...
        {
            var savePath = GetFileSaveDirectory(WitTexts.Texts.ConfigurationFileManagerLabel,
                WitTexts.Texts.ConfigurationFileNameLabel, "asset");
        ...
        public static int SaveConfiguration(string savePath, string serverToken, WitConfiguration configurationAsset)
        {
            ...
            if (string.IsNullOrEmpty(savePath)) return -1;
            // Must be in assets
            var unityPath = savePath.Replace("\\", "/");
            if (!unityPath.StartsWith(Application.dataPath))
            {
                Debug.LogError(
                    $"Configuration Utility - Cannot Create Configuration Outside of Assets Directory\nPath: {unityPath}");
                return -1;
            }
            ...
            var index = GetConfigurationIndex(name);
        ...
        public static void SetServerToken(string serverToken, Action<string> onSetComplete = null)
        {
            ...
            PerformRequest(listRequest, (r, o) => ApplyAllApplicationData(serverToken, r, o),
                error => { SetServerTokenComplete(serverToken, error, onSetComplete); });
        }
        ...
        public static void SetServerToken(this WitConfiguration configuration, string serverToken,
            Action<string> onSetComplete = null)
        {
            // Invalid
            if (!IsServerTokenValid(serverToken))
            {
        ...
        // Refresh client data
        private static void SetApplicationData(WitConfiguration configuration, string serverToken,
            Action<string> onSetComplete)
        {
            // Already set in app server data
            var appID = GetAppID(configuration);
            if (!string.IsNullOrEmpty(appID))
            {
                var curToken = WitAuthUtility.GetAppServerToken(appID);
        ...
        // Refresh client data
        private static void SetClientData(WitConfiguration configuration, string serverToken,
            Action<string> onSetComplete)
        {
            // Invalid app ID
            var appID = GetAppID(configuration);
            ...
            // Find client id
            PerformConfigRequest(configuration, configuration.GetClientToken(appID), ApplyClientToken,
                error => { SetConfigServerTokenComplete(configuration, serverToken, error, onSetComplete); });
        }

        // Complete
        private static void SetConfigServerTokenComplete(WitConfiguration configuration, string serverToken,
            string error, Action<string> onSetComplete)
        {
            // Failed
            if (!string.IsNullOrEmpty(error))
            {
        ...
        private static readonly List<string> refreshAppIDs = new();
        ...
        {
            PerformConfigRequest(configuration, configuration.ListTraitsRequest(), ApplyTraitList,
                error => { RefreshDataComplete(configuration, error, onRefreshComplete); });
        }

        // Refresh data complete
        private static void RefreshDataComplete(WitConfiguration configuration, string error,
            Action<string> onRefreshComplete)
        {
            // Get refresh id
            var appID = GetAppID(configuration);
            if (IsRefreshing(appID)) refreshAppIDs.Remove(appID);
            // Failed
            if (!string.IsNullOrEmpty(error))
            {
        ...
        private static void PerformConfigRequest(WitConfiguration configuration, WitRequest request,
            Action<WitConfiguration, WitResponseNode, Action<string>> onApply, Action<string> onComplete)
        {
            PerformRequest(request,
                (response, onRequestComplete) => { onApply(configuration, response, onRequestComplete); }, onComplete);
        }

        // Perform a wit request and then apply data
        private static void PerformRequest(WitRequest request, Action<WitResponseNode, Action<string>> onApply,
        ...
        private static void ApplyAllApplicationData(string serverToken, WitResponseNode witResponse,
            Action<string> onComplete)
        {
            ...
            for (var i = 0; i < applications.Count; i++)
            {
                ...
                var witConfigIndex = Array.FindIndex(WitConfigs,
                    configuration => string.Equals(appID, configuration?.application?.id));
                if (witConfigIndex != -1)
                {
                    var configuration = _witConfigs[witConfigIndex];
        ...
        private static void ApplyApplicationData(WitConfiguration configuration, WitResponseNode witResponse,
            Action<string> onComplete)
        {
            var applications = witResponse.AsArray;
            for (var i = 0; i < applications.Count; i++)
        ...
        // Apply client id
        private static void ApplyClientToken(WitConfiguration configuration, WitResponseNode witResponse,
            Action<string> onComplete)
        {
        ...
        // Apply intents
        private static void ApplyIntentList(WitConfiguration configuration, WitResponseNode witResponse,
            Action<string> onComplete)
        {
        ...
        // Apply entities
        private static void ApplyEntityList(WitConfiguration configuration, WitResponseNode witResponse,
            Action<string> onComplete)
        {
        ...
        // Apply traits
        private static void ApplyTraitList(WitConfiguration configuration, WitResponseNode witResponse,
            Action<string> onComplete)
        {
        ...
        // Update all
        private static void UpdateConfigItem(int index, WitConfigurationData[] items, WitConfiguration configuration,
            Action<string> onComplete)
        {
            // Complete
        ...
        {
            var l = "Wit Configuration Utility - " + comment;
            if (error) Debug.LogError(l);
#if VERBOSE_LOG
            else
            {
                Debug.Log(l);
