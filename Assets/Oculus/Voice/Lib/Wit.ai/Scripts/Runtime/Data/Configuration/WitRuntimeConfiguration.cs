...
namespace Facebook.WitAi.Configuration
{
    ...
    {
        [Tooltip("Configuration for the application used in this instance of Wit.ai services")] [SerializeField]
        ...
        [Tooltip("The length of the individual samples read from the audio source")] [Range(10, 500)] [SerializeField]
        ...
        [Tooltip("The maximum amount of concurrent requests that can occur")] [Range(1, 10)] [SerializeField]
        ...
        [Tooltip(
            "A custom provider that returns text to be used for nlu processing on activation instead of sending audio.")]
        [SerializeField]
        public CustomTranscriptionProvider customTranscriptionProvider;

        [Tooltip(
            "If always record is set the mic will fill the mic data buffer as long as the component is enabled in the scene.")]
        public bool alwaysRecord;

        [Tooltip(
            "The preferred number of seconds to offset from the time the activation happens. A negative value here could help to catch any words that may have been cut off at the beginning of an activation (assuming input is already being read into the buffer)")]
        public float preferredActivationOffset = -.5f;

        protected virtual Vector2 RecordingTimeRange => new(0, 20);
    }
}
