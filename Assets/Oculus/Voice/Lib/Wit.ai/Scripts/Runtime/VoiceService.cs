...
namespace Facebook.WitAi
{
    public abstract class VoiceService : MonoBehaviour, IVoiceService, IInstanceResolver, IAudioEventProvider
    {
        ...
        internal VoiceEvents events = new();
        ...
        {
            var conduitDispatcherFactory = new ConduitDispatcherFactory(this, conduitParameterProvider);
        ...
        {
            var witConfigProvider = GetComponent<IWitRuntimeConfigProvider>();
            _witConfiguration = witConfigProvider.RuntimeConfiguration.witConfiguration;

            InitializeEventListeners();

            if (!UseConduit) MatchIntentRegistry.Initialize();
        }

        protected virtual void OnEnable()
        {
            if (UseConduit) ConduitDispatcher.Initialize(_witConfiguration.manifestLocalPath);
        ...
        /// <summary>
        ///     Returns true if the service is actively communicating with Wit.ai during an Activation. The mic may or may not
        ///     still be active while this is true.
        /// </summary>
        ...
        /// <summary>
        ///     Start listening for sound or speech from the user and start sending data to Wit.ai once sound or speech has been
        ///     detected.
        /// </summary>
        public abstract void Activate();

        /// <summary>
        ///     Activate the microphone and send data for NLU processing. Includes optional additional request parameters like
        ///     dynamic entities and maximum results.
        /// </summary>
        /// <param name="requestOptions"></param>
        public abstract void Activate(WitRequestOptions requestOptions);

        /// <summary>
        ///     Activate the microphone and send data for NLU processing immediately without waiting for sound/speech from the user
        ///     to begin.
        /// </summary>
        ...
        /// <summary>
        ///     Activate the microphone and send data for NLU processing immediately without waiting for sound/speech from the user
        ///     to begin.  Includes optional additional request parameters like dynamic entities and maximum results.
        /// </summary>
        ...
        private void InitializeEventListeners()
        {
            ...
            if (!audioEventListener) gameObject.AddComponent<AudioEventListener>();

            var transcriptionEventListener = GetComponent<TranscriptionEventListener>();
            if (!transcriptionEventListener) gameObject.AddComponent<TranscriptionEventListener>();
        }
        ...
        protected virtual void ValidateShortResponse(WitResponseNode response)
        {
            if (VoiceEvents.OnValidatePartialResponse != null)
            {
                ...
                var validationData = new VoiceSession();
                ...
                if (UseConduit)
                {
                    ...
                    var intent = response.GetFirstIntentData();
                    if (intent != null)
                    {
                        var parameters = GetConduitResponseParameters(response);
        ...
        private void HandleIntents(WitResponseNode response)
        {
            ...
            foreach (var intent in intents) HandleIntent(intent, response);
        }
        ...
        private void HandleIntent(WitIntentData intent, WitResponseNode response)
        {
            if (UseConduit)
            {
                ConduitDispatcher.InvokeAction(intent.name, GetConduitResponseParameters(response), intent.confidence);
            }
            else
            {
                var methods = MatchIntentRegistry.RegisteredMethods[intent.name];
                foreach (var method in methods) ExecuteRegisteredMatch(method, intent, response);
            }
        }
            ...
            WitIntentData intent, WitResponseNode response)
        {
            if (intent.confidence >= registeredMethod.matchIntent.MinConfidence &&
                ...
                foreach (var obj in GetObjectsOfType(registeredMethod.type))
                {
                    ...
                    {
                        Debug.LogError(
                            "Match intent only supports methods with no parameters or with a WitResponseNode parameter. Enable Conduit or adjust the parameters");
                        continue;
                    }

                    if (parameters.Length == 1) registeredMethod.method.Invoke(obj, new object[] { response });
                }
        }
    }
