...
namespace Facebook.WitAi.CallbackHandlers
{
    ...
    public class SimpleIntentHandler : WitResponseHandler
    {
        ...
        [SerializeField] private UnityEvent onIntentTriggered = new();

        [Tooltip(
            "Confidence ranges are executed in order. If checked, all confidence values will be checked instead of stopping on the first one that matches.")]
        [SerializeField]
        ...
        protected override void OnHandleResponse(WitResponseNode response)
        {
            ...
            foreach (var intentNode in response?["intents"]?.Childs)
            {
                ...
                {
                    matched = true;
                    if (resultConfidence >= confidence) onIntentTriggered.Invoke();
        ...
        {
            for (var i = 0; null != confidenceRanges && i < confidenceRanges.Length; i++)
            {
