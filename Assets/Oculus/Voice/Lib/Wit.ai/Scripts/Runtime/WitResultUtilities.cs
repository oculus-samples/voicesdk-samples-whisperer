...
namespace Facebook.WitAi
{
    public static class WitResultUtilities
    {
        ...
        public static WitIntentData[] GetIntents(this WitResponseNode witResponse)
        {
            ...
            for (var i = 0; i < intents.Length; i++) intents[i] = intentResponseArray[i].AsWitIntent;
        ...
        public static WitEntityData[] GetEntities(this WitResponseNode witResponse, string name)
        {
            ...
            for (var i = 0; i < entities.Length; i++) entities[i] = entityJsonArray[i].AsWitEntity;
        ...
        public static WitEntityFloatData[] GetFloatEntities(this WitResponseNode witResponse, string name)
        {
            ...
            for (var i = 0; i < entities.Length; i++) entities[i] = entityJsonArray[i].AsWitFloatEntity;
        ...
        public static WitEntityIntData[] GetIntEntities(this WitResponseNode witResponse, string name)
        {
            ...
            for (var i = 0; i < entities.Length; i++) entities[i] = entityJsonArray[i].AsWitIntEntity;
        ...
        public static string GetPathValue(this WitResponseNode response, string path)
        {
            ...
            {
                var arrayElements = SplitArrays(nodeName);

                node = node[arrayElements[0]];
                for (var i = 1; i < arrayElements.Length; i++) node = node[int.Parse(arrayElements[i])];
            }
        ...
        {
            var nodes = path.Trim('.').Split('.');
            var code = "witResponse";
            foreach (var nodeName in nodes)
            {
                var arrayElements = SplitArrays(nodeName);

                code += $"[\"{arrayElements[0]}\"]";
                for (var i = 1; i < arrayElements.Length; i++) code += $"[{arrayElements[i]}]";
            }
        ...
        private static string[] SplitArrays(string nodeName)
        {
            ...
            for (var i = 0; i < nodes.Length; i++) nodes[i] = nodes[i].Trim(']');
    ...
    public class ArrayNodeReference : WitResponseReference
    {
        ...
        {
            if (null != child) return child.GetStringValue(response[index]);
        ...
        {
            if (null != child) return child.GetIntValue(response[index]);
        ...
        {
            if (null != child) return child.GetFloatValue(response[index]);
    ...
    public class ObjectNodeReference : WitResponseReference
    {
        ...
        public override string GetStringValue(WitResponseNode response)
        {
            if (null != child && null != response?[key]) return child.GetStringValue(response[key]);
        ...
        public override int GetIntValue(WitResponseNode response)
        {
            if (null != child) return child.GetIntValue(response[key]);
        ...
        public override float GetFloatValue(WitResponseNode response)
        {
            if (null != child) return child.GetFloatValue(response[key]);
