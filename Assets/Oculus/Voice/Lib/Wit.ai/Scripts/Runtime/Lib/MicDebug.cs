...
namespace Facebook.WitAi.Lib
{
    public class MicDebug : MonoBehaviour
    {
        ...
        {
            if (_micSource == null) _micSource = gameObject.GetComponentInChildren<IAudioInputSource>();
            if (_micSource != null)
            {
        ...
        private void OnStartRecording()
        {
            // Get root
#if UNITY_EDITOR
            var path = Application.dataPath.Replace("\\", "/").Replace("/Assets", "");
#else
            ...
            path =
                $"{path}/{fileName}{now.Year:0000}{now.Month:00}{now.Day:00}_{now.Hour:00}{now.Minute:00}{now.Second:00}.pcm";
        ...
        private void OnSampleReady(int sampleCount, float[] sample, float levelMax)
        {
            ...
            if (_fileStream == null || sample == null) return;
            ...
            {
                var data = (short)(sample[i] * FLOAT_TO_SHORT);
