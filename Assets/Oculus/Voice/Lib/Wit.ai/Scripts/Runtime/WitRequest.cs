...
namespace Facebook.WitAi
{
    ...
    public class WitRequest
    {
        ...
        /// </summary>
        public AudioEncoding audioEncoding = new();
        ...
        private readonly WitConfiguration configuration;

        private readonly bool configurationRequired;

        private bool isRequestStreamActive;

        private readonly bool isServerAuthRequired;
        ...
        private ConcurrentQueue<byte[]> writeBuffer = new();
            ...
            params QueryParam[] queryParams)
        {
            ...
            this.path = path;
            this.queryParams = queryParams;
            if (isServerAuthRequired) serverToken = WitAuthUtility.GetAppServerToken(configuration?.application?.id);
        }

        public WitRequest(string serverToken, string path, params QueryParam[] queryParams)
        {
            configurationRequired = false;
            isServerAuthRequired = true;
            command = path.Split('/').First();
            this.path = path;
            this.queryParams = queryParams;
        ...
        ///     called
        /// </summary>
        public WitResponseNode ResponseData { get; private set; }
        ...
        private void StartRequest(Uri uri)
        {
            ...
            if (onPreSendRequest != null) onPreSendRequest(ref uri, out customHeaders);

            // Create http web request
            _request = WebRequest.Create(uri.AbsoluteUri) as HttpWebRequest;
            if (Application.isBatchMode) _request.KeepAlive = false;
        ...
        public static string GetUserAgent(WitConfiguration configuration)
        {
            ...
            if (string.IsNullOrEmpty(configId)) configId = "not-yet-configured";

            // Append custom user agents
            var customUserAgents = string.Empty;
            if (OnProvideCustomUserAgent != null)
                foreach (Func<string> del in OnProvideCustomUserAgent.GetInvocationList())
                {
                    var custom = del();
                    if (!string.IsNullOrEmpty(custom)) customUserAgents += $",{custom}";
                }

            // Return full string
            return
                $"voice-sdk-45.0.0.4.140,wit-unity-{WIT_SDK_VERSION},{_operatingSystem},{_deviceModel},{configId},{_appIdentifier},{userEditor},{_unityVersion}{customUserAgents}";
        }
        ...
        private void StartResponse()
        {
            ...
            ThreadPool.RegisterWaitForSingleObject(asyncResult.AsyncWaitHandle, HandleTimeoutTimer, _request, Timeout,
                true);
        }
        ...
        private void HandleResponse(IAsyncResult asyncResult)
        {
            ...
            {
                // Final transcription
                var transcription = ResponseData.GetTranscription();
        ...
        private bool ProcessStringResponse(string stringResponse)
        {
            ...
            // Handle responses
            var hasResponse = ResponseData.HasResponse();
            var final = hasResponse && ResponseData.GetIsFinal();
            ...
            // Call final response
            if (final) SafeInvoke(onResponse);
        ...
        // Check performing
        private CoroutineUtility.CoroutinePerformer _performer;
        ...
        private IEnumerator PerformMainThreadCallbacks()
        {
            ...
            while (HasMainThreadCallbacks())
            {
                ...
                // Perform if possible
                while (_mainThreadCallbacks.Count > 0 && _mainThreadCallbacks.TryDequeue(out var result)) result();
            }
