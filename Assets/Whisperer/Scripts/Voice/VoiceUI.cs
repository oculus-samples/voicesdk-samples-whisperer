...
{
	/// <summary>
	/// Handles the canvas UI for each listenable object, displaying the mic input level, transcription, and response status
	/// </summary>
	public class VoiceUI : ListenableUI
	{
		...
		[SerializeField] InputActionReference _tooltipInputAction;

		[Header("Asset References")]
		[SerializeField] private Sprite _listeningIcon;
		...
		public string LastTranscriptionCache { get => _lastTranscription; }
		...
		private void RespondToVoiceVolume()
		{
			...
			float normalizedVolume = Mathf.Clamp(_micInputValue.AudioLevel * 30, 0f, 1f);
			Vector3 volumeScale = new Vector3(_initialVoiceStatusScale.x * normalizedVolume, _initialVoiceStatusScale.y * normalizedVolume, _initialVoiceStatusScale.z * normalizedVolume);
		...
		private void HandleTooltip()
		{
			...
			if (_listenable.IsSelected && _tooltipInputAction.action.ReadValue<float>() == 1)
			{
				ShowTooltip();
			}

			if (_listenable.IsSelected && _tooltipInputAction.action.ReadValue<float>() == 0)
			{
				HideTooltip();
			}
		}
		...
		private void SetTooltipText(UtteranceTooltipDefinition utteranceTooltips)
		{
			...
			List<string> randomUtterances = Utilities.RandomizeList(utteranceTooltips.exampleUtterances);
