...
namespace Whisperer
{
	...
	public class AudioManager : MonoBehaviour
	{
		...
		[SerializeField] AudioSource _narratorSource;
		[SerializeField] AudioSource _musicSource;
		[SerializeField] AudioSource _auxSource;
		[SerializeField] GameObject _spatialPrefab;
		[SerializeField] int _spatialPoolSize = 3;
		[SerializeField] List<SoundLib> _soundLibs;
		[SerializeField] Easings.Functions _easing;

		[SerializeField] List<Transform> _spatialSources = new List<Transform>();
		[SerializeField] List<IEnumerator> _coroutines = new List<IEnumerator>();
		[SerializeField] List<SoundLib.ClipData> _clipDatas = new List<SoundLib.ClipData>();

		[Header("Debug")]
		[SerializeField] int _voicesInUsePeak;
		...
		public float Play(string playID, Transform positionTransform, float volume = 1)
		{
			...
			SoundLib.ClipData clipData = FindData(playID);
		...
		{
			for (int i = 0; i < _clipDatas.Count; i++)
			{
				if (_clipDatas[i] != null && _clipDatas[i].PlayID == playID)
				{
					AudioSource source = _spatialSources[i].GetComponent<AudioSource>();
		...
		private void PlaySpatialSource(SoundLib.ClipData clipData, Transform positionTransform)
		{
			...
			AudioSource source = GetAudioSource(positionTransform);

			if (source)
			{
				int index = _spatialSources.IndexOf(source.transform);
		...
		{
			float volume = Mathf.Lerp(-80, 0, Easings.Interpolate(progress, _easing));		
